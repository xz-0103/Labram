...
class SEEDLoader(torch.utils.data.Dataset):
    def __init__(self, root, files, sampling_rate=200):
        ...
        self.root = root
        self.files = files
        self.default_rate = 200  # 默认采样率是200
...
def prepare_SEED_dataset(root):
    ...
    # 获取 train、val、test 文件夹中的文件列表
    train_files = os.listdir(os.path.join(root, "train"))
    np.random.shuffle(train_files)  # 随机打乱训练文件列表
    ...
    # 假设你有一个 SEEDLoader 类来处理数据加载
    train_dataset = SEEDLoader(os.path.join(root, "train"), train_files)
    val_dataset = SEEDLoader(os.path.join(root, "val"), val_files)
    test_dataset = SEEDLoader(os.path.join(root, "test"), test_files)
